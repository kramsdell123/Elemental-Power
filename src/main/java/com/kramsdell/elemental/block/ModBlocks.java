package com.kramsdell.elemental.block;

import com.kramsdell.elemental.ElementalPower;

import net.fabricmc.fabric.api.item.v1.FabricItemSettings;
import net.fabricmc.fabric.api.object.builder.v1.block.FabricBlockSettings;
import net.minecraft.block.Block;
import net.minecraft.block.Blocks;
import net.minecraft.block.piston.PistonBehavior;
import net.minecraft.item.BlockItem;
import net.minecraft.item.Item;
import net.minecraft.registry.Registries;
import net.minecraft.registry.Registry;
import net.minecraft.util.Identifier;

public class ModBlocks {
    public static final Block lit_obsidian = register("lit_obsidian", new Block(FabricBlockSettings.copyOf(Blocks.OBSIDIAN).pistonBehavior(PistonBehavior.BLOCK)), false);

    public static Item registerItem(String name, Block block) {
        return Registry.register(Registries.ITEM, name, new BlockItem(block, new FabricItemSettings()));
    }

    public static Block register(String name, Block block, boolean addBlockItem) {
        if(addBlockItem) {
            registerItem(name, block);
        }
        return Registry.register(Registries.BLOCK, new Identifier(ElementalPower.MOD_ID, name), block);
    }

    public static void registerBlocks() {
        ElementalPower.LOGGER.info("Registered Blocks");
    }
}
